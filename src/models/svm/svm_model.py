...
def train_svm(X_train, X_test, y_train, y_test, cv_folds=5):
    """Train the SVM Model."""
    ...
    plt.savefig(f"roc_curve_svm_{cv_folds}_folds.png")
    plt.show()
    ...
    plt.ylabel("Actual")
    plt.savefig(f"confusion_matrix_svm_{cv_folds}_folds.png")
    plt.show()
...
if __name__ == "__main__":
    try:
        ...
        for folds in [5, 7, 10]:
            ...
            best_model, X_train_transformed, X_test_transformed = train_svm(X_train, X_test, y_train, y_test, cv_folds=folds)
