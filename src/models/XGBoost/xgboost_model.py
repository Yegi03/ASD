...
def train_xgboost(X_train, X_test, y_train, y_test, cv_folds=5):
    ...
    plt.savefig(f"xgb_roc_curve_{cv_folds}_folds.png")
    plt.show()
    ...
    plt.ylabel("Actual")
    plt.savefig(f"xgb_confusion_matrix_{cv_folds}_folds.png")
    plt.show()
    ...
    plt.savefig(f"xgb_shap_summary_{cv_folds}_folds.png", dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    try:
        ...
        for folds in [5, 7, 10]:
            ...
            train_xgboost(X_train, X_test, y_train, y_test, cv_folds=folds)
